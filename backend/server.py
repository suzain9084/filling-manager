...
# from backend.filing_app.utils.gemini_api import GeminiAPI
# import ast
...
# def getSignatureData(text,i,image,isEnclosures,isVerification,isPrayer,court_type,signature):
...
#     if not isPrayer and 'PRAYER' in text:
#         isPrayer = True

#     if ("FILED THROUGH" in text) or ('APPLICANT' in text and (court_type == 'ngt' or court_type == 'cat')) or ('PETITIONER' in text and court_type == 'high_court'):
...
#                 if (court_type == 'ngt' or court_type == 'cat') and (isVerification or isEnclosures) or(court_type == 'high_court' and isPrayer):
#                     signature.append({
#                         'x': data['left'][j] - 5,
#                         'y': data['top'][j],
#                         'h': data['height'][j],
...
# def clean_pdf_via_images(input_path, output_path):
...
#     temp_input_path, ocr_output_path = makeTempFile(pdfs=[index])
...
#         temp_input_path,temp_advo_sig_path, temp_clie_sig_path, ocr_output_path = makeTempFile(pdfs=pdfs,images=imgs)
#     else:
#         temp_input_path, ocr_output_path = makeTempFile(pdfs=pdfs) 
...
#             if not isOrignal:
#                 getSignatureData(text,i,images[i],isEnclosures,isVerification,isPrayer,court_type,signature)    
...
#         for i, page in enumerate(reader.pages):
#             if not isOrignal:
#                 putSignatureAtplace(signature,i,page,img_width,img_height,temp_advo_sig_path,temp_clie_sig_path)
...
#     temp_input_path, ocr_output_path = makeTempFile(pdfs=[pdf_file])
...
#         )
#     except:
#         ocr_output_path = temp_input_path
...
#     if not isOrignal:
#         imgs = [advoSig]
#         temp_input_path,temp_advo_sig_path, _ = makeTempFile(pdfs=pdfs,images=imgs)
#     else:
#         temp_input_path, _ = makeTempFile(pdfs=pdfs) 
...
#     temp_input_path, _ = makeTempFile(pdfs=[pdf_file])
